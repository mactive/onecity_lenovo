...
/**
 * SINEMALL 广告管理程序    $Author: dolphin $
 * $Id: ads.php 14515 2008-04-28 08:36:23Z dolphin $
*/
...
elseif ($_REQUEST['act'] == 'insert')
{
	...
	elseif ($_POST['media_type'] == '4' )
    {
        ...
        {
            $ad_code = basename($image->upload_image($_FILES['ad_img4'], 'afficheimg'));
        }
        if (!empty($_POST['img_url']))
        {
            $ad_code = $_POST['img_url'];
        }
        if (((isset($_FILES['ad_img4']['error']) && $_FILES['ad_img4']['error'] > 0) || (!isset($_FILES['ad_img4']['error']) && isset($_FILES['ad_img4']['tmp_name']) && $_FILES['ad_img4']['tmp_name'] == 'none')) && empty($_POST['img_url']))
        {
            $link[] = array('text' => $_LANG['go_back'], 'href' => 'javascript:history.back(-1)');
            sys_msg($_LANG['js_languages']['ad_code_empty'], 0, $link);
        }
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    if ($ads_arr['media_type'] == '0' || $ads_arr['media_type'] == '4')
    {
...
elseif ($_REQUEST['act'] == 'update')
{
    ...
    if ($type == 4)
    {
        if ((isset($_FILES['ad_img4']['error']) && $_FILES['ad_img4']['error'] == 0) || (!isset($_FILES['ad_img4']['error']) && isset($_FILES['ad_img4']['tmp_name']) && $_FILES['ad_img4']['tmp_name'] != 'none'))
        {
            $img_up_info = basename($image->upload_image($_FILES['ad_img4'], 'afficheimg'));
            $ad_code = "ad_code = '".$img_up_info."'".',';
        }
        else
        {
...
function get_adslist()
{
    ...
    while ($rows = $GLOBALS['db']->fetchRow($res))
    {
         ...
         $rows['type'] .= ($rows['media_type'] == 4) ? $GLOBALS['_LANG']['ad_defineshop']  : '';
