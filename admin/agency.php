...
/**
 * SINEMALL 管理中心办事处管理    $Author: testyang $
...
$exc = new exchange($ecs->table('office'), $db, 'agency_id', 'agency_name');
...
elseif ($_REQUEST['act'] == 'batch')
{
    ...
    else
    {
        ...
        {
            /* 删除记录 */
            $sql = "DELETE FROM " . $ecs->table('office') .
...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit')
{
    ...
    else
    {
        ...
        $id = $_GET['id'];
        $sql = "SELECT * FROM " . $ecs->table('office') . " WHERE agency_id = '$id'";
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    if ($is_add)
    {
        $db->autoExecute($ecs->table('office'), $agency, 'INSERT');
        $agency['agency_id'] = $db->insert_id();
    }
    else
    {
        $db->autoExecute($ecs->table('office'), $agency, 'UPDATE', "agency_id = '$agency[agency_id]'");
    }
...
function get_agencylist()
{
    ...
    if ($result === false)
    {
        ...
        /* 查询记录总数，计算分页数 */
        $sql = "SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('office');
        $filter['record_count'] = $GLOBALS['db']->getOne($sql);
        $filter = page_and_size($filter);

        /* 查询记录 */
        $sql = "SELECT * FROM " . $GLOBALS['ecs']->table('office') . " ORDER BY $filter[sort_by] $filter[sort_order]";
