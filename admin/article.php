...
if ($_REQUEST['act'] == 'add')
{
    ...
    $article['is_open'] = 1;
    $article['templete'] = 4;
    $article['other_cat'] = array();
    

	// 扩展分类
    $other_cat_list = article_cat_list(0, 0);
...
if ($_REQUEST['act'] == 'insert')
{
    ...
    else
    {
	 ...
	 /*处理图片*/
    $img_name = basename($image->upload_image($_FILES['articlelogo'],'articlelogo'));
    ...
    $sql = "INSERT INTO ".$ecs->table('article')."(title, logo, cat_id, article_type, is_open, author, ".
                "author_email, keywords, flickr_rss, brief, content, add_time, templete,file_url, open_type, link) ".
            "VALUES ('$_POST[title]', '$img_name', '$_POST[article_cat]', '$_POST[article_type]', '$_POST[is_open]', ".
	...
	/* 处理相册图片 */
    handle_article_gallery_image($article_id, $_FILES['img_url'], $_POST['img_desc'],$_POST['img_sort']);
...
if ($_REQUEST['act'] == 'edit')
{
	...
	/* 图片列表 */
    $sql = "SELECT * FROM " . $ecs->table('articles_gallery') . " WHERE article_id = '$_REQUEST[id]' ORDER BY img_sort ASC";
    $img_list = $db->getAll($sql);
    $smarty->assign('img_list', $img_list);

    /* 取得关联资料 */
    $goods_article_list = get_goods_articles($_REQUEST['id']);   // 关联资料
	...
	*/
    ...
    $sql = "SELECT cat_id FROM " . $ecs->table('article_extend') . " WHERE article_id = '$_REQUEST[id]'";
...
if ($_REQUEST['act'] =='update')
{
	...
	/* 处理相册图片 */    
	handle_article_gallery_image($_POST['id'], $_FILES['img_url'], $_POST['img_desc'],$_POST['img_sort']);
    ...
    if (isset($_POST['old_img_sort']))
    {
        ...
        {
            $sql = "UPDATE " . $ecs->table('articles_gallery') . " SET img_sort = '$img_sort' WHERE img_id = '$img_id' LIMIT 1";
            $db->query($sql);
        }
    }
	...
	$param = " title='$_POST[title]', cat_id='$_POST[article_cat]', article_type='$_POST[article_type]', is_open='$_POST[is_open]', author='$_POST[author]', author_email='$_POST[author_email]', keywords ='$_POST[keywords]', flickr_rss ='$_POST[flickr_rss]',  brief ='$_POST[brief]', templete = '$_POST[templete]', file_url ='$file_url', open_type='$open_type', content='$_POST[FCKeditor1]', link='$_POST[link_url]'  ";
    if (!empty($img_name))
    {
        //有图片上传
        $param .= " ,logo = '$img_name' ";
