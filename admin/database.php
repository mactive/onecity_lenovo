...
/**
 * SINEMALL
 * 数据库管理    $Author: testyang $
 * $Id: database.php 14690 2008-06-28 03:35:43Z testyang $
*/
...
if ($_REQUEST['act'] == 'dumpsql')
{
    ...
    switch ($type)
    {
        ...
        case 'stand':
            $temp = array('admin_user','area_region','article','article_cat','attribute','brand','cart','category','comment','goods','goods_attr','goods_cat','goods_gallery','goods_type','group_goods','link_goods','similar_goods','member_price','order_action','order_goods','order_info','payment','region','shipping','shipping_area','shop_config','user_address','user_bonus','user_rank','users','virtual_card');
            foreach ($temp AS $table)
            {
        ...
        case 'min':
            $temp = array('attribute','brand','cart','category','goods','goods_attr','goods_cat','goods_gallery','goods_type','group_goods','link_goods','similar_goods','member_price','order_action','order_goods','order_info','shop_config','user_address','user_bonus','user_rank','users','virtual_card');
            foreach ($temp AS $table)
            {
                $tables[$ecs->prefix . $table] = -1;
            }
