...
/**
 * SINEMALL 商品管理程序    $Author: testyang $
...
if ($_REQUEST['act'] == 'list' || $_REQUEST['act'] == 'trash' || $_REQUEST['act'] == 'trash_tmp')
{
	...
	$smarty->assign('status_list',   get_goods_status_list());
    ...
    {
        case 'list':
            $goods_list = goods_list(0, (($code == '') ? 1 : 0));
            break;
        case 'trash':
        	$goods_list = goods_list(1, -1, -1);
            break;
        case 'trash_tmp':
    		$goods_list = goods_list(1, -1, 1243977982);
            break;
    }
	
    //$goods_list = goods_list($_REQUEST['act'] == 'list' ? 0 : 1, ($_REQUEST['act'] == 'list') ? (($code == '') ? 1 : 0) : -1, $_REQUEST['act'] == 'trash_tmp' ? 1 : 0 );
    ...
    $htm_file = ($_REQUEST['act'] == 'list') ?
        'goods_list.htm' : (($_REQUEST['act'] == 'trash'  || $_REQUEST['act'] == 'trash_tmp') ? 'goods_trash.htm' : 'group_list.htm');
...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit' || $_REQUEST['act'] == 'copy')
{
	...
	$max_goods_id     = $db->getOne("SELECT MAX(goods_id) + 1 FROM ".$ecs->table('goods'));
    ...
    if ($is_add)
    {
        ...
        $goods = array(
            ...
            'promote_price' => 0,
            'market_price'  => 0,
			'part_number'   => generate_part_number($max_goods_id),
            'salebase_price'=> 0,
            'agency_price'  => 0,
			'price_rate_level' => 0,
            'integral'      => 0,
		...
		/* 相似商品 */
        $similar_goods_list = array();
		$sql = "DELETE FROM " . $ecs->table('similar_goods') .
		        " WHERE (goods_id = 0 OR similar_goods_id = 0)" .
		        " AND admin_id = '$_SESSION[admin_id]'";
		$db->query($sql);
    ...
    else
    {
        ...
        if ($_REQUEST['act'] == 'copy')
        {
            ...
            $goods['goods_id'] = 0;
            $goods['goods_sn'] = '';
	        $goods['part_number'] = generate_part_number($max_goods_id);
            $goods['goods_name'] = '';
            $goods['goods_img'] = '';
            $goods['goods_thumb'] = '';
            $goods['original_img'] = '';
            ...
            // 相似商品
			            ...
			            $sql = "SELECT '0' AS goods_id, similar_goods_id, is_double, '$_SESSION[admin_id]' AS admin_id" .
			                    " FROM " . $ecs->table('similar_goods') .
			                    " WHERE goods_id = '$_REQUEST[goods_id]' ";
			            $res = $db->query($sql);
			            while ($row = $db->fetchRow($res))
			            {
			                $db->autoExecute($ecs->table('similar_goods'), $row, 'INSERT');
			            }

			            $sql = "SELECT goods_id, '0' AS similar_goods_id, is_double, '$_SESSION[admin_id]' AS admin_id" .
			                    " FROM " . $ecs->table('similar_goods') .
			                    " WHERE similar_goods_id = '$_REQUEST[goods_id]' ";
			            $res = $db->query($sql);
			            while ($row = $db->fetchRow($res))
			            {
			                $db->autoExecute($ecs->table('similar_goods'), $row, 'INSERT');
        ...
        $link_goods_list    = get_linked_goods($goods['goods_id']); // 相关商品
		$similar_goods_list = get_similared_goods($goods['goods_id']); // 相似商品
        $group_goods_list   = get_group_goods($goods['goods_id']); // 配件
        $goods_article_list = get_goods_articles($goods['goods_id']);   // 关联资料
	...
	$smarty->assign('price_rate_select_lib',         $_LANG['price_rate_level']); // 为了下拉菜单 0:shop 1:cat 2:brand
	$smarty->assign('price_rate_lib', array(
	                                'shop_price_rate' => shop_price_rate,
	                                'cat_price_rate' => cat_price_rate,
	                                'brand_price_rate' => brand_price_rate)
	                                );
	$smarty->assign('cat_price_rate',         get_cat_info($goods['cat_id']) );
	$smarty->assign('brand_price_rate',        get_brand_info($goods['brand_id']) 	);
	...
	$smarty->assign('similar_goods_list', $similar_goods_list);
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    if (empty($_POST['goods_sn']))
    {
        ...
        $part_number= generate_part_number($max_id);
    }
    else
    {
		$max_id     = $is_insert ? $db->getOne("SELECT MAX(goods_id) + 1 FROM ".$ecs->table('goods')) : $_REQUEST['goods_id'];
    ...
    $salebase_price = !empty($_POST['salebase_price']) ? $_POST['salebase_price'] : 0;
    $agency_price = !empty($_POST['agency_price']) ? $_POST['agency_price'] : 0;
    $price_rate_level = !empty($_POST['price_rate_level']) ? $_POST['price_rate_level'] : 0;
    ...
    $goods_status = empty($_POST['goods_status']) ? '' : intval($_POST['goods_status']);

	/* 暂时的语句 暂时在后台修改产品的成本价*/
	$average_price = !empty($_POST['average_price']) ? $_POST['average_price'] : 0;
    ...
    if ($is_insert)
    {
        ...
        if ($code == '')
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, part_number, " .
                    "cat_id, brand_id, goods_status, average_price, shop_price, market_price, salebase_price, agency_price, price_rate_level, is_promote, promote_price, " .
                    "promote_start_date, promote_end_date, goods_img, goods_thumb, original_img, keywords, goods_brief, " .
                    "seller_note, goods_weight, goods_number, warn_number, integral, give_integral, is_best, is_new, is_hot, " .
                    "is_on_sale, is_alone_sale, goods_desc, add_time, last_update, goods_type)" .
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$part_number', '$catgory_id', " .
                    "'$brand_id', '$goods_status', '$average_price', '$shop_price', '$market_price', '$salebase_price', '$agency_price', '$price_rate_level', '$is_promote','$promote_price', ".
    ...
    else
    {
        ...
        $sql = "UPDATE " . $ecs->table('goods') . " SET " .
                ...
                "price_rate_level = '$price_rate_level', " .
...
//-- 修改精品推荐状态
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'toggle_vip')
{
    check_authz_json('goods_manage');

    $goods_id       = intval($_POST['id']);
    $is_vip        = intval($_POST['val']);

    if ($exc->edit("is_vip = '$is_vip', last_update=" .gmtime(), $goods_id))
    {
        clear_cache_files();
        make_json_result($is_vip);
    }
}
...
elseif ($_REQUEST['act'] == 'drop_goods')
{
	...
	$db->query($sql);
    $sql = "DELETE FROM " . $ecs->table('similar_goods') . " WHERE goods_id = '$goods_id'";
    $db->query($sql);
    $sql = "DELETE FROM " . $ecs->table('similar_goods') . " WHERE similar_goods_id = '$goods_id'";
	$db->query($sql);
...
//-- 把商品加入相似
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'add_similar_goods')
{
    include_once(ROOT_PATH . 'includes/cls_json.php');
    $json = new JSON;
    ...
    $similared_array   = $json->decode($_GET['add_ids']);
    $similared_goods   = $json->decode($_GET['JSON']);
    $goods_id       = $similared_goods[0];
    $is_double      = $similared_goods[1] == true ? 0 : 1;

    foreach ($similared_array AS $val)
    {
        if ($is_double)
        {
            /* 双向关联 */
            $sql = "INSERT INTO " . $ecs->table('similar_goods') . " (goods_id, similar_goods_id, is_double, admin_id) " .
                    "VALUES ('$val', '$goods_id', '$is_double', '$_SESSION[admin_id]')";
            $db->query($sql, 'SILENT');
        }
    ...
    $similared_goods   = get_similared_goods($goods_id);
    $options        = array();

    foreach ($similared_goods AS $val)
    {
        $options[] = array('value'  => $val['goods_id'],
                        'text'      => $val['goods_name'],
                        'data'      => '');
...
//-- 删除相似商品
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'drop_similar_goods')
{
    include_once(ROOT_PATH . 'includes/cls_json.php');
    $json = new JSON;
    ...
    $similared_goods   = $json->decode($_GET['JSON']);
    $goods_id       = $similared_goods[0];
    $is_signle      = $similared_goods[1];

    if (!$is_signle)
    {
        $sql = "DELETE FROM " .$ecs->table('similar_goods') .
                " WHERE similar_goods_id = '$goods_id' AND goods_id " . $drop_goods_ids;
    }
    else
    {
        $sql = "UPDATE " .$ecs->table('similar_goods') . " SET is_double = 0 ".
                " WHERE similar_goods_id = '$goods_id' AND goods_id " . $drop_goods_ids;
    }
    if ($goods_id == 0)
    {
    ...
    $sql = "DELETE FROM " .$ecs->table('similar_goods') .
            " WHERE goods_id = '$goods_id' AND similar_goods_id " . $drop_goods_ids;
    if ($goods_id == 0)
    {
        $sql .= " AND admin_id = '$_SESSION[admin_id]'";
    }
