...
/**
 * SINEMALL 后台自动操作数据库的类文件    $Author: testyang $
...
class exchange
{
    ...
    /**
     * 判断表中某字段是否重复，若重复则中止程序，并给出错误信息 为了判断step的重复而添加
     *
     * @access  public
     * @param   string  $col    字段名
     * @param   string  $name   字段值
     ...
     * @return void
     */
    function is_only_step($col, $name, $id = 0, $where='')
    {
        $sql = 'SELECT COUNT(*) FROM ' .$this->table. " WHERE $col = '$name'";
        $sql .= empty($id) ? '' : ' AND parent_id' . " = '$id'";
        $sql .= empty($where) ? '' : ' AND ' .$where;
		echo $sql;
        return ($this->db->getOne($sql) == 0);
    }
     ...
     * @return   int        记录个数
     */
    function sum($col, $name, $id = 0,$where='',$count='*')
    {
        $sql = 'SELECT SUM('.$count.') FROM ' .$this->table. " WHERE $col = '$name'";
        $sql .= empty($id) ? '' : ' AND '. $this->id ." != '$id' ";
		$sql .= empty($where) ? '' : ' AND ' .$where;
        return $this->db->getOne($sql);
    }
