...
/**
 * SINEMALL 管理中心公用文件    $Author: dolphin $
    ...
    $_REQUEST['act'] != 'forget_pwd' && $_REQUEST['act'] != 'reset_pwd' && $_REQUEST['act'] != 'check_order')
{
    ...
    if (!empty($_COOKIE['ECSCP']['admin_id']) && !empty($_COOKIE['ECSCP']['admin_pass']))
    {
        ...
        $sql = 'SELECT user_id, user_name, password, action_list, menu_list, last_login ' .
        ...
        else
        {
            ...
            if (md5($row['password'] . $_CFG['hash_code']) == $_COOKIE['ECSCP']['admin_pass'])
            {
                ...
                set_admin_session($row['user_id'], $row['user_name'], $row['action_list'], $row['menu_list'], $row['last_time']);
