 ...
 * @return  void
 */
function handle_similar_goods($goods_id)
{
    $sql = "UPDATE " . $GLOBALS['ecs']->table('similar_goods') . " SET " .
            " goods_id = '$goods_id' " .
            " WHERE goods_id = '0'" .
            " AND admin_id = '$_SESSION[admin_id]'";
    $GLOBALS['db']->query($sql);
    ...
    $sql = "UPDATE " . $GLOBALS['ecs']->table('similar_goods') . " SET " .
            " similar_goods_id = '$goods_id' " .
            " WHERE similar_goods_id = '0'" .
            " AND admin_id = '$_SESSION[admin_id]'";
    $GLOBALS['db']->query($sql);
}
...
function delete_goods($goods_id)
{
	...
	$GLOBALS['db']->query($sql);
    $sql = "DELETE FROM " . $GLOBALS['ecs']->table('similar_goods') . " WHERE goods_id " . db_create_in($goods_id);
    $GLOBALS['db']->query($sql);
    $sql = "DELETE FROM " . $GLOBALS['ecs']->table('similar_goods') . " WHERE similar_goods_id " . db_create_in($goods_id);
    $GLOBALS['db']->query($sql);
...
/**
 * 为某商品生成唯一的part_number
 * @param   int     $goods_id   商品编号
 * @return  string  唯一的货号
 */
function generate_part_number($goods_id)
{
    return "osc".$goods_id;
}
/**
 ...
 * @return  array
 */
function get_similared_goods($goods_id)
{
    $sql = "SELECT lg.similar_goods_id AS goods_id, g.goods_name, lg.is_double " .
            "FROM " . $GLOBALS['ecs']->table('similar_goods') . " AS lg, " .
                $GLOBALS['ecs']->table('goods') . " AS g " .
            "WHERE lg.goods_id = '$goods_id' " .
            "AND lg.similar_goods_id = g.goods_id ";
    if ($goods_id == 0)
    {
        $sql .= " AND lg.admin_id = '$_SESSION[admin_id]'";
    }
    ...
    foreach ($row AS $key => $val)
    {
        $similared_type = $val['is_double'] == 0 ? $GLOBALS['_LANG']['single'] : $GLOBALS['_LANG']['double'];

        $row[$key]['goods_name'] = $val['goods_name'] . " -- [$similared_type]";
 ...
 * @return  array
 */
function goods_list($is_delete, $real_goods=1,$update_time = -1)
{
    /* 过滤条件 */
    $param_str = '-' . $is_delete . '-' . $real_goods. '-' . $is_on_sale;
    ...
    if ($result === false)
    {
        ...
        {
            $where .= " AND last_update >= '$filter[start_time]'";
        }
        if ($filter['end_time'])
        {
            $where .= " AND last_update <= '$filter[end_time]'";
        }
		...
		if($update_time == -1){
			...
			$where .= " AND last_update ='$update_time'";
		}
        ...
        $sql = "SELECT goods_id, goods_name, goods_sn, part_number, shop_price,average_price,salebase_price, sort_order, is_on_sale, is_best, is_new, is_hot,is_vip, goods_number, integral, last_update," .
    ...
    {
        $row[$key]['last_update'] = local_date('Y-m-d H:i:s', $row[$key]['last_update']);
    }
