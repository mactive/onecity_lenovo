...
/**
 * SINEMALL 管理中心公用函数库    $Author: testyang $
 ...
 * @return  void
 */
function set_admin_session($user_id, $username, $action_list, $menu_list, $last_time)
{
    $_SESSION['admin_id']    = $user_id;
    $_SESSION['admin_name']  = $username;
    $_SESSION['action_list'] = $action_list;
    $_SESSION['menu_list']   = $menu_list;
...
function admin_menu($priv_str)
{
    ...
    if (strpos(',' . $_SESSION['menu_list'] . ',', ',' . $priv_str . ',') === false)
    {
        return false;
    }
    else
...
{
    $userList = array();
    $sql = 'SELECT user_id, real_name, primary_part FROM ' . $GLOBALS['ecs']->table('users');

    $sql .= ' ORDER BY  CONVERT( real_name USING gbk )   ASC';

    $res = $GLOBALS['db']->query($sql);

    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        $userList[$row['user_id']] = $row['real_name'] ." - ".$row['primary_part'];
    }
 ...
 */
function create_html_editor($input_name, $input_value = '',$editer_name ='FCKeditor')
{
    global $smarty;
    ...
    $smarty->assign($editer_name, $FCKeditor);
}
 ...
 * @param   object  $filters    过滤条件
 */
function get_solution_step_info($filter)
{
    $filter->keyword = json_str_iconv($filter->keyword);
    $where = get_where_step_sql($filter); // 取得过滤条件
	//$where = " WHERE step_id = $step_id";
    /* 取得数据 */
    $sql = 'SELECT * '.
           'FROM ' . $GLOBALS['ecs']->table('solution_step') . $where ;
 ...
 * @return  string
 */
function get_where_step_sql($filter)
{
    $where .= isset($filter->step_id) && $filter->step_id > 0 ? ' WHERE step_id = ' . $filter->step_id : '';
 ...
 * @param   object  $filters    过滤条件
 */
function get_json_goods_info($filter)
{
    $filter->keyword = json_str_iconv($filter->keyword);
    $where = get_where_goods_sql($filter); // 取得过滤条件
	//$where = " WHERE step_id = $step_id";
    /* 取得数据 */
    $sql = 'SELECT * '.
 ...
 * @return  string
 */
function get_where_goods_sql($filter)
{
    $where .= isset($filter->goods_id) && $filter->goods_id > 0 ? ' WHERE goods_id = ' . $filter->goods_id : '';
...
/* 获得制定等级的客户*/
function get_users_by_rankname($rank_name)
{
	$where = 'WHERE 1 ';
	if($rank_name){
		$where .= " AND o.rank_name LIKE '%" . mysql_like_quote($rank_name) . "%'";
	}

	$sql = 'SELECT '.
				'o.rank_id,o.rank_name,' .
				'u.user_rank,u.user_id,u.user_name'.
				
            " FROM " . $GLOBALS['ecs']->table('user_rank') . " AS o ".
            " LEFT JOIN " .$GLOBALS['ecs']->table('users') ." AS u ON u.user_rank=o.rank_id " .
            "$where". " ORDER BY u.user_name ASC ";
    ...
    {
		 $ranks[$row[$key]['user_id']] = $row[$key]['user_name'];
	}
	//print_r($ranks);
	return $ranks;
}
