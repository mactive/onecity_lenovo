...
/**
 * SINEMALL 管理中心网站设置    $Author: testyang $
 * $Id: shop_config.php 14511 2008-04-28 06:48:59Z testyang $
 */
...
if ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit')
{
    ...
    $smarty->assign('ur_here',      $_LANG['01_index_config']);
    ...
    $smarty->assign('brand_list', get_brand_list());
		...
		//print_r($config);
		/*      
		create_html_editor('index_topic_ad',	$config['index_topic_ad'],'edit_topic_ad');
        create_html_editor('index_topic_pic',	$config['index_topic_pic'],'edit_topic_pic');
        create_html_editor('index_goods_series',$config['index_goods_series'],'edit_goods_series');
        create_html_editor('index_news',$config['index_news'],'edit_news');
        create_html_editor('index_solutions',$config['index_solutions'],'edit_solutions');
        create_html_editor('index_cases',$config['index_cases'],'edit_cases');
		*/
    ...
    {	
        create_html_editor('topic_ad',	'','edit_topic_ad');
        create_html_editor('topic_pic',	'','edit_topic_pic');
        create_html_editor('goods_series','','edit_goods_series');
        create_html_editor('news','','edit_news');
        create_html_editor('solutions','','edit_solutions');
        create_html_editor('cases','','edit_cases');

        $smarty->assign('act', "insert");
    }
	*/
    $smarty->display('index_config.htm');
}
/*插入更新*/
elseif ($_REQUEST['act'] == 'update')
{
	...
	$sql = "SELECT code FROM " . $ecs->table('shop_config') . " WHERE parent_id = '$parent_id'";
    $arr = $db->getAll($sql);
	foreach($arr as $key => $val)
    {
		$tt = $val['code'];
		$sql = "UPDATE " . $ecs->table('shop_config') .
	         	" SET value = '$_POST[$tt]' " .
	            " WHERE code = '$tt' AND parent_id ='$parent_id' LIMIT 1 ";
	    $db->query($sql);
		//echo $sql."==".$tt."==";
    }
    ...
    $links[] = array('href' => 'index_config.php', 'text' =>  $_LANG['index_config']);
