 ...
 * 创建一个可编辑区
 */
listTable.edit = function(obj, act, id, need_refresh)
{ 
  var tag = obj.firstChild.tagName;
  var refresh = need_refresh ? need_refresh: 0 ; //是否需要执行 query 刷新
 ...
 * 创建一个可编辑区
 */
listTable.check_and_edit = function(obj, act, id, salebase_price,shop_price)
{ 
  var tag = obj.firstChild.tagName;
  if (typeof(tag) != "undefined" && tag.toLowerCase() == "input")
  {
  ...
  txt.onblur = function(e)
  {
    if (Utils.trim(txt.value).length > 0)
    {
	  	if(Utils.trim(txt.value) < salebase_price)
		{
			alert("单项价格不能低于销售底价！");
		}
		else{
 ...
 * 创建一个可编辑区
 */
listTable.select_edit = function(obj, act, order_id, need_refresh)
{ 
  ...
  */
  ...
  /* 编辑区失去焦点的处理函数 */
    ...
    if (Utils.trim(selected.text).length > 0 && order_id)
    {
      res = Ajax.call(listTable.url, "act="+act+"&val=" + encodeURIComponent(obj.value) + "&id=" +order_id, null, "POST", "JSON", false);

	  if(refresh){
		  listTable.loadList();
		}
	  
	  if(refresh>2){//need_fresh =3 只刷新上半部分 两个select
		if (res.content)
	      {
			  document.getElementById('so_page_header').innerHTML = res.content;
	      }
		}
