...
/**
 * SINEMALL 管理员信息以及权限管理程序    $Author: testyang $
...
elseif ($_REQUEST['act'] == 'signin')
{
    ...
    /* 检查密码是否正确 */
    $sql = "SELECT user_id, user_name, password, last_login, action_list, menu_list, last_login".
    ...
    {
            // 登录成功
        set_admin_session($row['user_id'], $row['user_name'], $row['action_list'], $row['menu_list'],  $row['last_login']);
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    {
        $sql = "SELECT agency_name FROM " . $ecs->table('office') . " WHERE agency_id = '$user_info[agency_id]'";
...
elseif ($_REQUEST['act'] == 'allot')
{
    ...
    $menu_str = $db->getOne("SELECT menu_list FROM " .$ecs->table('admin_user'). " WHERE user_id = '$_GET[id]'");
    ...
    foreach ($priv_arr AS $action_id => $action_group)
    {
       ...
       // $priv_arr[$action_id]['menu_list'] = join(',', @array_keys($action_group['priv']));

		//$priv_arr[$action_id]['priv']['canmenu'] = (strpos($menu_str, $val['action_menu']) !== false ) ? 1 : 0;			
...
elseif ($_REQUEST['act'] == 'update_allot')
{
    ...
    $menu_list = @join(",", $_POST['menu_list']);

    $sql = "UPDATE " .$ecs->table('admin_user'). 
			" SET action_list = '$act_list', ".
				"menu_list = '$menu_list' ".
