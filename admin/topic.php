...
/**
 * SINEMALL 产品系列管理    $Author: testyang $
 * $Id: topic.php 14668 2008-06-10 08:17:16Z testyang $
*/
...
$exc   = new exchange($ecs->table("topic"), $db, 'topic_id', 'title');
/* act操作项的初始化 */
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
	...
	/*处理图片*/    
    $img_name = basename($image->upload_image($_FILES['topic_logo'],'topiclogo'));

    if ($is_insert)
    {
        $sql = "INSERT INTO " . $ecs->table('topic') . " (title,logo,brief,topic_tag,start_time,end_time,data,intro,template,css)" .
                "VALUES ('$_POST[topic_name]','$img_name','$_POST[topic_brief]','$_POST[topic_tag]','$start_time','$end_time','$data','$_POST[topic_intro]','$_POST[topic_template_file]','$_POST[topic_css]')";
    }
    else
    {
		
        $sql = "UPDATE " . $ecs->table('topic') .
                "SET title='$_POST[topic_name]',brief='$_POST[topic_brief]',topic_tag='$_POST[topic_tag]',start_time='$start_time',end_time='$end_time',data='$data',intro='$_POST[topic_intro]',template='$_POST[topic_template_file]',css='$_POST[topic_css]'" ;
		...
		$sql .= " WHERE topic_id='$topic_id' LIMIT 1";
    }
	//echo $sql;
    $db->query($sql);
    ...
    $links[] = array('href' => 'topic.php?act=edit&topic_id='.$topic_id, 'text' =>  $_LANG['back_list']);
