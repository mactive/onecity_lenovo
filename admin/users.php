 ...
 * ============================================================================
 * $Author: testyang $
 * $Id: users.php 14640 2008-06-04 01:48:33Z testyang $
*/
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    $sql = "SELECT u.user_name, u.sex, u.birthday, u.pay_points, u.rank_points, u.user_rank , u.user_money, u.frozen_money, u.credit_line, u.parent_id, u2.user_name as parent_username, u.qq,u.real_name,u.msn, u.office_phone, u.mobile_phone ".
    ...
    $sql = "SELECT u.user_id, u.sex, u.birthday, u.pay_points, u.rank_points, u.user_rank , u.user_money, u.frozen_money, u.credit_line, u.parent_id, u2.user_name as parent_username, u.qq, u.real_name, u.msn,
    u.office_phone, u.mobile_phone ".
        " FROM " .$ecs->table('users'). " u LEFT JOIN " . $ecs->table('users') . " u2 ON u.parent_id = u2.user_id WHERE u.user_id='$_GET[id]'";
...
//-- 导出状态库存统计表
/*------------------------------------------------------ */
elseif($_REQUEST['act'] == 'users_xls')
{	
	$file_name = "users".local_date('Y-m-d H:i:s', gmtime());
	
    $user_list = user_list($_REQUEST['limit_num']);
	//print_r($user_list);
	$title = array(	'user_id'=>'用户ID',
					'user_name'=>'用户名',
					'email' => 'Email', 
					'sex'=>'性别',
					'birthday'=>'生日',
	...
	if(excel_write_with_sub_array($file_name,$title,$user_list['user_list']))
	{
		$link[0]['text'] = $_LANG['profile_lnk'];
	    $link[0]['href'] = 'inventory.php?act=status_accounting';

		$link[1]['text'] = '下载';
	    $link[1]['href'] = '../xls/admin/'.$file_name.'.xls';
	
	    sys_msg('xls/admin/'.$file_name.'.xls 导出成功', 0, $link,false);
	}
}
...
function user_list($limit_num = '')
{
    ...
    if ($result === false)
    {
		...
		$limit_sql = $limit_num == 'all' ? '' : " LIMIT " . $filter['start'] . ',' . $filter['page_size'];
