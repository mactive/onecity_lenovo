...
$size = isset($_CFG['page_size'])  && intval($_CFG['page_size']) > 0 ? intval($_CFG['page_size']) : 15;
...
/* 页面的缓存ID */
$cache_id = sprintf('%X', crc32($cat_id . '-' . $display . '-' . $sort  .'-' . $order  .'-' . $page . '-' . $size . '-' . $_SESSION['user_id'] . '-' .
...
if (!$smarty->is_cached('category.dwt', $cache_id))
{
    ...
    if ($cat['grade'] > 1)
    {
        ...
        */
        ...
        foreach ($price_grade as $key=>$val)
        {
            ...
            $price_grade[$key]['url'] = build_uri('solution_operate', array('cid'=>$cat_id, 'bid'=>0, 'price_min'=>$price_grade[$key]['start'], 'price_max'=> $price_grade[$key]['end'], 'filter_attr'=>0));
    ...
    if ($cat['filter_attr'] > 0)
    {
        ...
        {
            $attr_list[$key]['url'] = build_uri('solution_operate', array('cid'=>$cat_id, 'bid'=>0, 'price_min'=>0, 'price_max'=>0, 'filter_attr'=>$val['attr_value']));
    ...
    $smarty->assign('sub_categories',   get_sub_categories_tree($cat_id)); // 子分类树
    $smarty->assign('helps',            get_shop_help());              // 网店帮助
    ...
    $brand_list = array_merge($arr, get_brands($cat_id, 'category','1'));
...
//	$smarty->assign('hot_data',       get_recommend_goods_cate_hot());     // 热点商品
	...
	$smarty->assign('chave_no_subcategory',       have_no_subcategory($cat_id));

if(have_no_subcategory($cat_id)){
	$smarty->display('category.dwt', $cache_id);
}else{
	$smarty->display('category.dwt', $cache_id);
//	$smarty->display('category_no_goods.dwt', $cache_id);
}
...
function category_get_goods($children, $brand, $min, $max, $ext, $size, $page, $sort, $order)
{
    ...
    $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style, g.market_price, g.is_new, g.is_best, g.is_hot, g.shop_price AS org_price, ' .
                ...
                'g.promote_start_date, g.promote_end_date, g.goods_brief,g.goods_desc, g.goods_thumb , g.goods_img ' .
            'FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' .
            'LEFT JOIN ' . $GLOBALS['ecs']->table('member_price') . ' AS mp ' .
                "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' " .
            "WHERE $where $ext ORDER BY g.is_promote DESC, g.is_best DESC , $sort $order";
