				...
				if(CITY_AD_LIMIT - $city_ad_num > 0)
				{
					if($city_confirm_ad_num >= 2){
						...
						if($tmp['col_18'] < 2){
							...
							$sql ="UPDATE " . $GLOBALS['ecs']->table($GLOBALS['year']."_".'category') . " SET renew_upload = '1'  WHERE cat_id = '$val[city_id]'";
...
elseif($_REQUEST['act'] == 'update_audit')
{
	...
	if($confirm == 1){
		...
		$sql = "UPDATE " . $GLOBALS['ecs']->table($GLOBALS['year']."_".'category') . " SET audit_status = '$audit_status'  WHERE cat_id = '$city_id'";
        $GLOBALS['db']->query($sql);
