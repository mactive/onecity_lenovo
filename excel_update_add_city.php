...
{ 
	...
	for ($row=2;$row<=$all_sheets[$y]['numRows'];$row++) 
	{
		...
		$city = trim($all_sheets[$y]['cells'][$row][3]);
		...
		$parent_id = 2638;//get_cat_id_by_name($city);
		
		
		if($parent_id){
			$sql = "INSERT INTO " . $GLOBALS['ecs']->table($GLOBALS['year']."_".'category') .
					" (`cat_name`,`parent_id`,`level`, `sys_level`,`market_level` ) ".
					"VALUES ('$county', '$parent_id','$level','5','$market_level')";
			echo $sql."<br>";
			...
			$sql_2 = "INSERT INTO " . $GLOBALS['ecs']->table('city_request') .
					" (`city_id`,`back_city_name`,`parent_id`,`sys_level` ) ".
					"VALUES ('$insert_id', '$county', '$parent_id', '5')";
			echo $sql_2."<br>";
		    $GLOBALS['db']->query($sql_2);
		
			$sql_3 = "INSERT INTO " . $GLOBALS['ecs']->table('city_resource') .
					" (`city_id`,`back_city_name`,`parent_id`,`sys_level`, `market_level`, `resource` , `Q1`, `Q2`, `Q3`, `Q4` ) ".
					"VALUES ('$insert_id', '$county', '$parent_id', '5', '$market_level', '1', '1', '6', '1', '2')";
			echo $sql_3."<br>";
