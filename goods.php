...
/**
 * SINEMALL 商品详情 * $Author: testyang $
 * $Id: goods.php 14637 2008-05-30 06:16:27Z testyang $
*/
...
if (!empty($_REQUEST['act']) && $_REQUEST['act'] == 'price')
{
    ...
    else
    {
		...
		$agency_price =  $goods_info['agency_price'];	    
		$market_price =  ($goods_info['promote_price_org'] > 0) ? $goods_info['promote_price_org'] : $goods_info['market_price'];
        $shop_price  = ($goods_info['promote_price_org'] > 0) ? $goods_info['promote_price_org'] : $goods_info['rank_price'];
        $shop_price += $attr_amount;

		$res['result'] = $_SESSION['user_rank'] == 5 ? price_format($agency_price * $number):price_format($shop_price * $number);
		// VIP 会员是 agency_price
...
if (!$smarty->is_cached('goods.dwt', $cache_id))
{
    ...
    else
    {
        ...
        /* meta */
        $smarty->assign('keywords',           htmlspecialchars($goods['goods_name'])); //原为 $goods['keywords'] 
        ...
        $smarty->assign('attribute_linked',    get_same_attribute_goods($properties));           // 相同属性的相关商品
        $smarty->assign('related_goods',       $linked_goods);                                   // 相关商品
        $smarty->assign('similared_goods',     get_similared_goods($goods_id));                    // 相似
		$smarty->assign('goods_article_list',  get_linked_articles($goods_id));                  // 关联资料
        $smarty->assign('fittings',            get_goods_fittings($goods_id));                   // 配件
        $smarty->assign('rank_prices',         get_user_rank_prices($goods_id, $shop_price));    // 员工级别价格
        ...
        //$tag_linked_data = get_linked_tags($tag_data);
...
/* 更新点击次数 */
$sql = 'UPDATE ' . $ecs->table('goods') . " SET click_count = click_count + 1 WHERE goods_id = '$_REQUEST[id]'";
$db->query($sql);
...
function get_similared_goods($goods_id)
{
    $sql = 'SELECT g.goods_id, g.goods_name, g.goods_thumb, g.goods_img, g.shop_price AS org_price, ' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, ".
                'g.market_price, g.promote_price, g.promote_start_date, g.promote_end_date ' .
            'FROM ' . $GLOBALS['ecs']->table('similar_goods') . ' lg ' .
            'LEFT JOIN ' . $GLOBALS['ecs']->table('goods') . ' AS g ON g.goods_id = lg.similar_goods_id ' .
            "LEFT JOIN " . $GLOBALS['ecs']->table('member_price') . " AS mp ".
                    "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' ".
            "WHERE lg.goods_id = '$goods_id' AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 ".
            "LIMIT " . $GLOBALS['_CFG']['related_goods_number'];
