    ...
    function select($tag)
    {
        ...
        else
        {
            ...
            switch ($tag_sel)
            {
				...
				case 'random' :
                    $t = $this->get_para(substr($tag, 7), 0);
					$res = explode(',', $t['values']);
					$tmp = array('var'=>'int_rand','value'=>'');
					$tmp['value'] = rand($res[0],$res[1]);
                    /* 如果传进来的值是变量，就不用用引号 */
                    $tmp = '$this->assign(\'' . $tmp['var'] . '\',' . $tmp['value'] . ');';
                    

                    return '<?php ' . $tmp . ' ?>';
                    break;
                    /*
                    return '<?php echo $this->random(' . $this->make_array($t) . '); ?>';
                    break;
					*/
