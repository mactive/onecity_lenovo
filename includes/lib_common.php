 ...
 * @return  void
 */
function solution_options($spec_cat_id, $arr)
{
        ...
        while (!empty($arr))
        {
            foreach ($arr AS $key => $value)
            {
                ...
                if ($level == 0 && $last_cat_id == 0)
                {
                    ...
                    $options[$cat_id]['name']  = $value['solution_name'];
                    unset($arr[$key]);
 ...
 * @return array 品牌列表 id => name
 */
function get_goods_status_list()
{
    $sql = 'SELECT status_id, status_name FROM ' . $GLOBALS['ecs']->table('goods_status') .' WHERE is_show = 1'. ' ORDER BY sort_order';
    $res = $GLOBALS['db']->getAll($sql);

    $status_list = array();
    foreach ($res AS $row)
    {
        $status_list[$row['status_id']] = $row['status_name'];
    }
 ...
 * @return  array
 */
function get_brands($cat = 0, $app = 'brand', $iscommend = 1, $limitnum = 0)
{
...
function send_mail($name, $email, $subject, $content, $type = 0, $notification=false,$mail_from = '')
{
	$mail_from = $mail_from ? $mail_from : $GLOBALS['_CFG']['smtp_mail'];
    ...
    if ($GLOBALS['_CFG']['mail_charset'] != EC_CHARSET)
    {
        ...
        $content   = ecs_iconv(EC_CHARSET, $GLOBALS['_CFG']['mail_charset'], $content);//这有字符限制？
    ...
    else
    {
        ...
        $headers[] = 'From: "' . '=?' . $charset . '?B?' . base64_encode($GLOBALS['_CFG']['shop_name']) . '?='.'" <' . $mail_from . '>';
...
function build_uri($app, $params, $append = '', $page = 0, $size = 0)
{
		...
		case 'solution_operate':
            if (empty($cid))
            {
				if (!empty($bid))
                {
                    if(!empty($oid))
					{
						$uri = 'solution_operate.php?brand=' . $bid .'&order_id='.$oid;
					}else{
						$uri = 'solution_operate.php?brand=' . $bid;
					}
            ...
            else
            {
                if ($rewrite)
                {
					//为solution_operate 单独写 rewriteg规则
	/*				*/
                    $uri = 'solution_operate.php?category=' . $cid;
                    if (isset($bid))
                    {
                        $uri .= '&brand=' . $bid;
                    }
        ...
        case 'idea':
            ...
            {
                $uri = $rewrite ? 'idea-' . $aid : 'idea.php?id=' . $aid;
            }
    ...
    if ($rewrite && $app!='solution_operate')
    {
    ...
    else
    {
        if (stristr($filename, ROOT_PATH) === false)
        {
            if ($extname == 'jpg' || $extname == 'jpeg' || $extname == 'gif' || $extname == 'png' || $extname == 'doc' ||
                ...
                $extname == 'swf' || $extname == 'chm'  || $extname == 'sql' || $extname == 'cert'|| $extname == '7z')
            {
                $format = $extname;
            }
        }
...
function desc_to_brief($goods_desc)
{
	$tmp = substr($goods_desc,stripos($goods_desc,"</P>"));
	
	$tmp = str_ireplace("<p>"," ",$tmp);
    $tmp = str_ireplace("<br/>"," ",$tmp);
    $tmp = str_ireplace("img"," ",$tmp);
    $tmp = str_ireplace("&nbsp;"," ",$tmp);
    $tmp = str_ireplace("size","si",$tmp);
	return $tmp;
}
/////---- about article function ----//////
/**
 * 取得资料列表：用于商品关联资料
...
function article_cat_list($cat_id = 0, $selected = 0, $re_type = true, $level = 0)
{
    ...
    if ($res === NULL)
    {
        $sql = "SELECT c.*, COUNT(s.cat_id) AS has_children, COUNT(a.article_id) AS aricle_num , ur.rank_name ".
               ...
               " LEFT JOIN " . $GLOBALS['ecs']->table('user_rank') . " AS ur ON ur.rank_id=c.cat_user_rank".
               " GROUP BY c.cat_id ".
               " ORDER BY parent_id, sort_order ASC";
        $res = $GLOBALS['db']->getAllCached($sql);
    }
...
{
    /* 查询现有的扩展分类 */
    $sql = "SELECT cat_id FROM " . $GLOBALS['ecs']->table('article_extend') .
    ...
    if ($delete_list)
    {
        $sql = "DELETE FROM " . $GLOBALS['ecs']->table('article_extend') .
                " WHERE article_id = '$article_id' " .
    ...
    {
        // 插入记录
        $sql = "INSERT INTO " . $GLOBALS['ecs']->table('article_extend') .
                " (article_id, cat_id) " .
 ...
 * @return  mix
 */
function article_extend_cat_list($cat_id = 0, $selected = 0, $re_type = true, $level = 0, $is_show_all = true)
{
    static $res = NULL;
    ...
    if ($res === NULL)
    {
        ...
        $sql = "SELECT c.cat_id as cat_id, COUNT(g.article_id) AS goods_num ".
                'FROM ' . $GLOBALS['ecs']->table('article_cat') . " AS c ".
                "LEFT JOIN " . $GLOBALS['ecs']->table('article') . " AS g ON g.cat_id=c.cat_id ".
                "GROUP BY c.cat_id ";
        $res2 = $GLOBALS['db']->getAllCached($sql);
    ...
    else
    {
        foreach ($options AS $key => $value)
        {
            $options[$key]['url'] = build_uri('article_cat', array('cid' => $value['cat_id']), $value['cat_name']);
        }
...
function get_article_children_a ($cat = 0)
{
 ...
 * @return  void
 */
function handle_article_gallery_image($article_id, $image_files, $image_descs,$image_sorts)
{
    foreach ($image_descs AS $key => $img_desc)
    {
        ...
        if ($flag)
        {
			...
			$img_id = $image_sorts[$key];

            $sql = "INSERT INTO " . $GLOBALS['ecs']->table('articles_gallery') . " (article_id, img_url, img_desc, img_sort, thumb_url, img_original) " .
                    "VALUES ('$article_id', '$img_url', '$img_desc', '$img_sort', '$thumb_url', '$img_original')";
			$GLOBALS['db']->query($sql);
        }
    }
}
 ...
 * @return  mix
 */
function idea_cat_list($cat_id = 0, $selected = 0, $re_type = true, $level = 0)
{
    static $res = NULL;

    if ($res === NULL)
    {
        $sql = "SELECT c.*, COUNT(s.cat_id) AS has_children, COUNT(a.idea_id) AS aricle_num , ur.rank_name ".
               ' FROM ' . $GLOBALS['ecs']->table('idea_cat') . " AS c".
               " LEFT JOIN " . $GLOBALS['ecs']->table('idea_cat') . " AS s ON s.parent_id=c.cat_id".
               " LEFT JOIN " . $GLOBALS['ecs']->table('idea') . " AS a ON a.cat_id=c.cat_id".
               " LEFT JOIN " . $GLOBALS['ecs']->table('user_rank') . " AS ur ON ur.rank_id=c.cat_user_rank".
               " GROUP BY c.cat_id ".
               " ORDER BY parent_id, sort_order ASC";
        $res = $GLOBALS['db']->getAllCached($sql);
    ...
    $options = idea_cat_options($cat_id, $res); // 获得指定分类下的子分类的数组
    ...
    else
    {
        foreach ($options AS $key => $value)
        {
            $options[$key]['url'] = build_uri('idea_cat', array('acid' => $value['cat_id']), $value['cat_name']);
        }
        return $options;
    }
}
...
/**
 * 保存某商品的扩展分类
 * @param   int     $idea_id   商品编号
 * @param   array   $cat_list   分类编号数组
 * @return  void
 */
function handle_idea_other_cat($idea_id, $cat_list)
{
    /* 查询现有的扩展分类 */
    $sql = "SELECT cat_id FROM " . $GLOBALS['ecs']->table('idea_extend') .
    ...
    if ($delete_list)
    {
        $sql = "DELETE FROM " . $GLOBALS['ecs']->table('idea_extend') .
                " WHERE idea_id = '$idea_id' " .
                "AND cat_id " . db_create_in($delete_list);
        $GLOBALS['db']->query($sql);
    }
    ...
    {
        // 插入记录
        $sql = "INSERT INTO " . $GLOBALS['ecs']->table('idea_extend') .
                " (idea_id, cat_id) " .
                "VALUES ('$idea_id', '$cat_id')";
        $GLOBALS['db']->query($sql);
    }
}
/**
 ...
 * @return  void
 */
function idea_cat_options($spec_cat_id, $arr)
{
    static $cat_options = array();
 ...
 * @return  mix
 */
function idea_extend_cat_list($cat_id = 0, $selected = 0, $re_type = true, $level = 0, $is_show_all = true)
{
    static $res = NULL;
    ...
    if ($res === NULL)
    {
        ...
        $sql = "SELECT c.cat_id as cat_id, COUNT(g.idea_id) AS goods_num ".
                'FROM ' . $GLOBALS['ecs']->table('idea_cat') . " AS c ".
                "LEFT JOIN " . $GLOBALS['ecs']->table('idea') . " AS g ON g.cat_id=c.cat_id ".
                "GROUP BY c.cat_id ";
        $res2 = $GLOBALS['db']->getAllCached($sql);
    ...
    else
    {
        foreach ($options AS $key => $value)
        {
            $options[$key]['url'] = build_uri('idea_cat', array('cid' => $value['cat_id']), $value['cat_name']);
        }
 ...
 * @return void
 */
function get_idea_children ($cat = 0)
{
    return db_create_in(array_unique(array_merge(array($cat), array_keys(idea_cat_list($cat, 0, false)))), 'cat_id');
}

function get_idea_children_a ($cat = 0)
{
    return db_create_in(array_unique(array_merge(array($cat), array_keys(idea_cat_list($cat, 0, false)))), 'a.cat_id');
}
/**
 * 取得资料列表：用于商品关联资料
 * @param   object  $filters    过滤条件
 */
function get_idea_list($filter)
{
    /* 创建数据容器对象 */
    $ol = new OptionList();
    ...
    /* 取得数据 */
    $sql = 'SELECT a.idea_id, a.title '.
           'FROM ' .$GLOBALS['ecs']->table('idea'). ' AS a, ' .$GLOBALS['ecs']->table('idea_cat'). ' AS c ' . $where;
    $res = $GLOBALS['db']->query($sql);

    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        $ol->add_option($row['idea_id'], $row['title']);
    }
