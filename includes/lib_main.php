...
/**
 * SINEMALL 前台公用函数库 * $Author: testyang $
 * $Id: lib_main.php 14650 2008-06-05 15:00:47Z testyang $
*/
...
function update_user_info()
{
    ...
    /* 查询会员信息 */
    $time = date('Y-m-d');
    $sql = 'SELECT u.user_money, u.pay_points, u.user_rank, u.rank_points, u.real_name, '.
    ...
    if ($row = $GLOBALS['db']->getRow($sql))
    {
        ...
        if ($row['user_rank'] == 0)
        {
            ...
            if ($row = $GLOBALS['db']->getRow($sql))
            {
			    ...
			    $_SESSION['user_rank_name'] = $GLOBALS['db']->getOne("SELECT rank_name FROM " . $GLOBALS['ecs']->table('user_rank') . " WHERE rank_id = '$row[rank_id]'");
...
function get_user_info($id=0)
{
    ...
    $time = date('Y-m-d');
    $sql  = 'SELECT u.user_id, u.email, u.user_name, u.user_money, u.user_money AS user_money_orginal, u.pay_points, u.real_name'.
            ' FROM ' .$GLOBALS['ecs']->table('users'). ' AS u ' .
            " WHERE u.user_id = '$id'";
	...
	$user['user_money_orginal'] = intval($user['user_money_orginal']);
    return $user;
}
 ...
 * @return array        $user       用户信息数组
 */
function get_contact_info_detail($id=0)
{
    if ($id == 0)
    {
        $id = $_SESSION['user_id'];
    ...
    $sql  = 'SELECT u.* '.
            ' FROM ' .$GLOBALS['ecs']->table('agency_contact'). ' AS u ' .
			//' LEFT JOIN '.$GLOBALS['ecs']->table('user_address'). ' AS ad ON ad.user_id = u.user_id' .
            " WHERE u.contact_id = '$id'";
 ...
 *
 * @access  public
 * @param   integer     $cat    分类编号（只有商品及分类、资料及分类用到）
 * @param   string      $str    商品名、资料标题或其他附加的内容（无链接）
 * @return  array
 */
function assign_ur_here($cat = 0, $str = '')
{
	...
	if (in_array($filename, array('category', 'goods'))) //产品和分类是产品直销开头
    {
    	$ur_here    = '<li><a href="mall.php">' . $GLOBALS['_LANG']['home'] . '</a></li>';
	}
    /* 根据文件名分别处理中间的部分 */
    if ($filename != 'index')
    {
        ...
        {	
            ...
            /* 循环分类 */
			if(in_array($filename, array('article_cat', 'article')) && !empty($cat_arr))
			{
				krsort($cat_arr);
        ...
        else
        {
            ...
            elseif ('topic' == $filename)
            {
                ...
                $args       = array('wsid' => '0');
                $ur_here   .= ' <li> <a href="topic.php">' .
                                $GLOBALS['_LANG']['topic'] . '</a></li>';
            }
			/* 批发 */
            elseif ('solution' == $filename)
            {
                $page_title = $GLOBALS['_LANG']['solution'] . '_' . $page_title;
                $args       = array('wsid' => '0');
                $ur_here   = ' <li> <a href="solution.php">' .
                                $GLOBALS['_LANG']['solution'] . '</a></li>';
            }
			/* 批发 */
            elseif ('contact_us' == $filename)
            {
                $page_title = $GLOBALS['_LANG']['contact_us'] . '_' . $page_title;
                $args       = array('wsid' => '0');
                $ur_here   = ' <li> <a href="contact_us.php">' .
                                $GLOBALS['_LANG']['contact_us'] . '</a></li>';
            }
			/* 批发 */
            elseif ('case' == $filename)
            {
                $page_title = $GLOBALS['_LANG']['case'] . '_' . $page_title;
                $args       = array('wsid' => '0');
                $ur_here   = ' <li> <a href="case.php">' .
                                $GLOBALS['_LANG']['case'] . '</a></li>';
            }
			/* 批发 */
            elseif ('training' == $filename)
            {
                $page_title = $GLOBALS['_LANG']['training'] . '_' . $page_title;
                $args       = array('wsid' => '0');
                $ur_here   = ' <li> <a href="training.php">' .
                                $GLOBALS['_LANG']['training'] . '</a></li>';
            }
			/* 配单 */
            elseif ('solution_operate' == $filename)
            {
                $page_title = $GLOBALS['_LANG']['solution_operate'] . '_' . $page_title;
                $args       = array('wsid' => '0');
                $ur_here   = ' <li> <a href="solution_operate.php">' .
                                $GLOBALS['_LANG']['solution_operate'] . '</a></li>';
            }
			/* 客户关系 */
            elseif ('crm' == $filename)
            {
                $page_title = $GLOBALS['_LANG']['crm'] . '_' . $page_title;
                $args       = array('wsid' => '0');
                $ur_here   = ' <li> <a href="crm.php">' .
                                $GLOBALS['_LANG']['crm'] . '</a></li>';
            }
            /* 其他的在这里补充 */
        }
    }
...
function build_urhere($arr, $type = 'category')
{
    ...
    foreach ($arr AS $val)
    {
        ...
        $str .= ' <code>tt</code> <a href="' . build_uri($type, $args). '">' . htmlspecialchars($val['cat_name']) . '</a>';
    }
 ...
 *
 * @access  public
 * @param   integer     $id     资料分类的编号
 * @param   integer     $num    资料数量
 * @return  array
 */
                        ...
                        $keywords = '', $filter_attr='')
{
    ...
    switch ($app)
    {
		...
		case 'solution_operate':
            $uri_args = array('cid' => $cat, 'bid' => $brand,'order_id' => $order);
            break;
        case 'article_cat':
...
{
    return array(
        'best'    => $GLOBALS['_LANG']['is_best'],
        'new'     => $GLOBALS['_LANG']['is_new'],
        'hot'     => $GLOBALS['_LANG']['is_hot'],
        'vip'	 => $GLOBALS['_LANG']['is_vip'],
        'promote' => $GLOBALS['_LANG']['is_promote'],
    );
}
...
function get_hot_info_list()
{
	$sql = 'SELECT a.article_id,a.title, ac.cat_user_rank FROM ' . $GLOBALS['ecs']->table('article') . ' AS a '.
			' LEFT JOIN ' .$GLOBALS['ecs']->table('article_cat'). ' AS ac ON a.cat_id = ac.cat_id '.
	 		" WHERE a.is_open = 1 " .
			" ORDER BY a.download_click DESC LIMIT 0,9 ";
 ...
 * @return  integer
 */
function get_goods_rank_common($goods_id)
{
