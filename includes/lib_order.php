...
/**
 * SINEMALL 购物流程函数库 * $Author: testyang $
...
function order_fee($order, $goods, $consignee)
{
    ...
    foreach ($goods AS $val)
    {
		...
		if(!$val['is_promote']){
			$total['no_promote_goods_price']  += $val['goods_price'] * $val['goods_number'];
		}
    ...
    $total['no_promote_goods_price_formated']  = price_format($total['no_promote_goods_price'], false);
...
//    $total['no_promote_goods_price']  = $total['no_promote_goods_price'];
...
{
    $sql = "SELECT rec_id, user_id, goods_id, goods_name, goods_sn, goods_number, is_promote," .
 ...
 * @return  boolean
 */
function addto_cart($goods_id, $num = 1, $spec = array(), $parent = 0,$buy_price)
{
    $GLOBALS['err']->clean();

    /* 取得商品信息 */
    $sql_now = "SELECT g.goods_name, g.goods_sn, g.is_on_sale, g.is_real, g.is_promote,".
    ...
    $goods = $GLOBALS['db']->getRow($sql_now);
	...
	*/
    ...
    {
        $num = $goods['shop_price'];
        $GLOBALS['err']->add(sprintf($GLOBALS['_LANG']['shop_price_age'], $num), ERR_OUT_OF_PRICE);
