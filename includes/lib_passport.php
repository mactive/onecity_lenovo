...
/**
 * SINEMALL 用户帐号相关函数库 * $Author: testyang $
 * $Id: lib_passport.php 14625 2008-05-26 10:07:21Z testyang $
*/
 ...
 * @return  bool         $bool
 */
function register($username, $password, $email, $other = array(),$take_register)
{
    /* 检查username */
    if (empty($username))
    {
    ...
    else
    {
        ...
        //定义other合法的变量数组
        $other_key_array = array('msn', 'qq', 'office_phone', 'real_name', 'mobile_phone','user_rank','is_validated','agency_id');
		...
		if($take_register){
			//print_r($update_data);
			//刚刚注册的用户
			$max_user_id =  $GLOBALS['db']->getOne("SELECT MAX(user_id) FROM ". $GLOBALS['ecs']->table('users'));			
        	$GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('users'), $update_data, 'UPDATE', 'user_id = ' .$max_user_id );
		}else{
			$GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('users'), $update_data, 'UPDATE', 'user_id = ' .$_SESSION['user_id'] );  			
			/*自动进行邮件验证 并设置权限
			$sql = "UPDATE " . $GLOBALS['ecs']->table('users') . " SET is_validated = 1 , user_rank = ".rank_webuser." WHERE user_id='$_SESSION[user_id]'";
            $GLOBALS['db']->query($sql);
			*/
			
			update_user_info();     // 更新用户信息
	        recalculate_price();    // 重新计算购物车中的商品价格     
	 		send_regiter_hash($_SESSION['user_id']);	//发送注册邮件
		}
