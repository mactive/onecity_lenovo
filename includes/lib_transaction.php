...
/**
 * SINEMALL 用户交易相关函数库 * $Author: testyang $
 * $Id: lib_transaction.php 14625 2008-05-26 10:07:21Z testyang $
*/
...
function edit_profile($profile)
{
    ...
    /* 过滤非法的键值 */
    $other_key_array = array('msn', 'qq', 'office_phone', 'real_name', 'mobile_phone');	
    foreach ($profile['other'] as $key => $val)
    {
        ...
        {
            $profile['other'][$key] =  htmlspecialchars($val); //防止用户输入javascript代码 而且会保留中文字符
        }
    }
    /* 修改在其他资料 */
...
/* 判断用户级别 */
function judge_user_rank($profile)
{
	if($user_rank > 4){
		break;
		return false;
	...
	if (!empty($profile['email']))
	{
		if (!empty($profile['other']['real_name']) && !empty($profile['other']['mobile_phone']) ){
			if(has_payed_orders($profile['user_id'])){
				$user_rank_level = rank_client;
			}else{
				$user_rank_level = rank_preclient;
			}
		}
		else{
			$user_rank_level = rank_webuser;	
		}
	}
	...
	$sql = 	"UPDATE " . $GLOBALS['ecs']->table('users') . 
			" SET user_rank = ".$user_rank_level." WHERE user_id = '$profile[user_id]'";
...
//是否有付款成功的订单 返回数量
function has_payed_orders($user_id){
	$sql = 	"SELECT count(*) FROM " . $GLOBALS['ecs']->table('order_info') . 
			" WHERE user_id = '$user_id' AND pay_status = ". PS_PAYED ;
	//echo $sql;
...
function get_profile($user_id)
{
    ...
    $sql  = "SELECT user_name, birthday, sex, question, answer, rank_points, pay_points,user_money, user_rank,  ".
             " msn, qq, office_phone, real_name, mobile_phone ".
 ...
 */
function get_user_solutions($user_id, $num = 10, $start = 0)
{
    /* 取得订单列表 */
    $arr    = array();

    $sql = "SELECT  o.* , s.solution_name , u.contact_name" .
           " FROM " .$GLOBALS['ecs']->table('solution_order') . " AS o " .
		   " LEFT JOIN ".$GLOBALS['ecs']->table('solution'). ' AS s ON s.solution_id = o.solution_id '.
		   " LEFT JOIN ".$GLOBALS['ecs']->table('agency_contact'). ' AS u ON u.contact_id = o.contact_id '.
           " WHERE o.user_id = '$user_id' AND o.is_delete = 0 ORDER BY add_time DESC";
	//echo $sql;
    ...
    {
		$row['handler'] = "<a href=\"solution.php?order_id=" .$row['order_id']."&solution_id=".$row['solution_id']."&act=edit". '">' .$GLOBALS['_LANG']['edit']. '</a> ';

        $arr[] = array('order_id'       => $row['order_id'],
            			'solution_id'      => $row['solution_id'],
            			'contact_id'      => $row['contact_id'],
                       'add_time'     	=> local_date($GLOBALS['_CFG']['date_format'], $row['add_time']),
