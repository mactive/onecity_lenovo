...
/**
 * SINEMALL 首页文件 * $Author: testyang $
 * $Id: index.php 14481 2008-04-18 11:23:01Z testyang $
*/
...
if (!$smarty->is_cached('index.dwt', $cache_id))
{
	...
	/*首页上的动态信息*/
	$sql = "SELECT * FROM " . $GLOBALS['ecs']->table('shop_config') . " WHERE parent_id = 100 ";
    $arr = $GLOBALS['db']->getAll($sql);
	
	//print_r($arr);
	foreach($arr as $key => $val)
    {
		$tt = $val['code'];  //code 名字 value 信息
		$smarty->assign($tt, 	$val['value']); //指定首页上的分块信息
    }
	...
	$smarty->assign('articles_list',    get_cat_articles(0, $page, $size,160));//资料列表
