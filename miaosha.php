...
/*------------------------------------------------------ */
/* 缓存编号 */
if($_REQUEST['act'] == "is_over")
{
	/* 获得商品的信息 */
    $goods_id = empty($_REQUEST['goods_id']) ? 0 : $_REQUEST['goods_id'] ;
    $goods_number = $db->getOne("SELECT goods_number FROM " . $GLOBALS['ecs']->table('goods') . " WHERE goods_id='$goods_id'");

	echo intval($goods_number);
	exit;
}
elseif($_REQUEST['act'] == "now_time")
{    
    $time = gmtime();
...
elseif($_REQUEST['act'] == "last_time")
{
	/* 获得商品的信息 */
	$goods_id = $GLOBALS['db']->getOne("SELECT goods_id FROM " . $GLOBALS['ecs']->table('goods') . " WHERE is_on_sale='1' ");
    $goods = get_goods_info($goods_id);
    
    $time = gmtime();

	echo intval($goods['gmt_end_time'] - gmtime());
	exit;
}
...
$smarty->assign('max_jinbi',         intval($user_info['user_money_orginal']));

if (!$smarty->is_cached('miaosha.dwt', $cache_id))
{
    assign_template();
    ...
    $smarty->assign('past_time',         intval(gmtime() - $goods['gmt_start_time']) );

    //$smarty->assign('past_time',     	intval( gmtime() － $goods['gmt_start_time'] ));
    //$smarty->assign('left_time',        intval( $goods['gmt_end_time'] - gmtime() ));
...
$smarty->display('miaosha.dwt', $cache_id);
