...
/**
 * SINEMALL 搜索程序 * $Author: testyang $
 * $Id: search.php 14633 2008-05-29 12:34:07Z testyang $
*/
...
//-- 高级搜索
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'intro_search')
{
    $goods_type = !empty($_REQUEST['goods_type']) ? intval($_REQUEST['goods_type']) : 0;
    $attributes = get_seachable_attributes($goods_type);
    $smarty->assign('goods_type_selected', $goods_type);
    ...
    $smarty->assign('action',     'intro_form');
    $smarty->assign('use_storage', $_CFG['use_storage']);
...
//    $_REQUEST['intro_type'] = !empty($_REQUEST['intro_type'])   ? intval($_REQUEST['intro_type'])   : 0;
    ...
    $action = '';
    if (isset($_REQUEST['action']) && $_REQUEST['action'] == 'form' || $_REQUEST['action'] == 'intro_form')
    {
        /* 要显示高级搜索栏 */
    ...
    if (!empty($_REQUEST['keywords']))
    {
        ...
        foreach ($arr AS $key => $val)
        {
            ...
            $keywords  .= "(goods_name LIKE '%$val%' OR goods_sn LIKE '%$val%' OR g.goods_id = '$val' OR keywords LIKE '%$val%' $sc_dsad)";
    ...
    if (!empty($_REQUEST['intro']))
    {
        switch ($_REQUEST['intro'])
        {
			...
			//vip产品
			case 'vip':
                $intro   = ' AND g.is_vip = 1';
                $intromode = $_SESSION['user_rank'] == 5 ? 'vip' : '';
                $ur_here = $_LANG['vip_goods'];
                break;
	...
	if($sort == 'sort_order')
	{
		$orser_sql = "ORDER BY g.is_promote DESC, g.is_best DESC , $sort $order";
	}else{
		$orser_sql = "ORDER BY $sort $order";
	}
	
	
    /* 查询商品 */
    $sql = "SELECT g.goods_id, g.goods_name, g.market_price, g.agency_price, g.is_new, g.is_best, g.is_hot, g.shop_price AS org_price, ".
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, ".
                "g.promote_price, g.promote_start_date, g.promote_end_date, g.goods_thumb, g.goods_img, g.goods_brief, g.goods_desc, g.goods_type ".
    ...
    while ($row = $db->FetchRow($res))
    {
        ...
        $arr[$row['goods_id']]['agency_price']  = $row['agency_price'];
		...
		$arr[$row['goods_id']]['anency_price_formated']  = price_format($row['agency_price']);
    ...
    $pager = array(
                ...
                'array'        => array(),
				'page_number' => 'search.php?keywords='.stripslashes($_REQUEST['keywords']),
