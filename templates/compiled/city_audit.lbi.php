...
<?php if ($this->_var['ad_detail']['is_new']): ?>
...
<form method="post" action="city_operate.php" name="theForm" id="theForm" enctype="multipart/form-data" onsubmit="return validate()">
  <table width="100%" border="0" cellspacing="0" class="table_standard">
	<tr>
		<td>已经上传照片</td>
		<td>
			<?php $_from = $this->_var['photo_info']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('', 'item_0_83695500_1335533706');$this->_foreach['photo_info'] = array('total' => count($_from), 'iteration' => 0);
if ($this->_foreach['photo_info']['total'] > 0):
    foreach ($_from AS $this->_var['item_0_83695500_1335533706']):
        $this->_foreach['photo_info']['iteration']++;
?>
				<div style="width:160px;height:160px;text-align:center;float:left;margin:10px;">
				<a href="<?php echo $this->_var['item_0_83695500_1335533706']['img_url']; ?>" target="_blank" class="city_photo"><img src="<?php echo $this->_var['item_0_83695500_1335533706']['thumb_url']; ?>"></a>
				<?php echo $this->_var['lang']['city_photo'][$this->_var['item_0_83695500_1335533706']['img_sort']]; ?>
				</div>
...
<?php if ($this->_var['sm_session']['user_rank'] > 1): ?>
	<?php if ($this->_var['sm_session']['user_rank'] == 2): ?>
		...
		<?php $_from = $this->_var['city_title']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('k', 'item_0_83773900_1335533706');if (count($_from)):
    foreach ($_from AS $this->_var['k'] => $this->_var['item_0_83773900_1335533706']):
?>
		<div class="city_info radius_5px">
			<div class="f_left left_title left_radius_5px"><?php echo $this->_var['item_0_83773900_1335533706']; ?></div>
		...
		<div style="width:500px;float:left;">
			...
			<?php $_from = $this->_var['city_title']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('k', 'item_0_83828200_1335533706');if (count($_from)):
    foreach ($_from AS $this->_var['k'] => $this->_var['item_0_83828200_1335533706']):
?>
	...
	<?php else: ?>
		<?php $_from = $this->_var['city_title']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('k', 'item_0_83846100_1335533706');if (count($_from)):
    foreach ($_from AS $this->_var['k'] => $this->_var['item_0_83846100_1335533706']):
?>
		<div class="city_info radius_5px">
			<div class="f_left left_title left_radius_5px"><?php echo $this->_var['item_0_83846100_1335533706']; ?></div>
