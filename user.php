...
/**
 * SINEMALL 会员中心 * $Author: testyang $
 * $Id: user.php 14724 2008-07-14 06:36:29Z testyang $
*/
...
$not_login_arr =
array('message_list','act_login','register','act_register','act_edit_password','get_password','send_pwd_email','password', 'signin', 'add_tag', 'collect', 'return_to_cart', 'logout', 'email_list', 'validate_email', 'send_hash_mail', 'order_query', 'is_registered', 'check_email','send_register','act_send_register');

/* 显示页面的action列表 */
$ui_arr = array('register', 'login', 'profile','bank_profile', 'order_list','solution_list', 'order_detail', 'address_list', 'collection_list',
'my_solution','tag_list', 'get_password', 'reset_password', 'booking_list', 'add_booking', 'account_raply','take_register',
...
if ($action == 'default')
{
	...
	$smarty->assign('page_title',      $_LANG['five_year']);     // 页面标题
    
	$smarty->assign('salesman_notice', $_CFG['salesman_notice']);
...
elseif ($action == 'act_register')
{
    ...
    $take_register    = isset($_POST['take_register']) ? trim($_POST['take_register']) : '0';
    ...
    if (register($username, $password, $email, $other, $take_register) !== false)
    {
...
{	
    ...
    if ($hash)
    {
        ...
        if ($id > 0)
        {
            $sql = "UPDATE " . $ecs->table('users') . " SET is_validated = 1 , user_rank = ".rank_webuser." WHERE user_id='$id'";
            $db->query($sql);
...
{		
	...
	/*不支持的邮箱*/
	//$no_support_email = array('sina.com','sohu.com');
	$no_support_email = array('1.com','2.com');
	$pos = 0;
	foreach($no_support_email AS $key)
	{
		$pos+= strpos($email,$key);
	}
...
elseif ($action == 'act_login')
{
    ...
    if ($user->login($username, $password))
    {
		...
		ecs_header('Location: city_renew.php'. "\n");
        exit;
...
/* 个人银行资料资料页面 */
elseif ($action == 'bank_profile')
{
    include_once(ROOT_PATH . 'includes/lib_transaction.php');

    $smarty->assign('bank_profile', get_profile($user_id));
    $smarty->display('user_transaction.dwt');
}
...
elseif ($action == 'act_edit_profile')
{
    ...
    $real_name = $_POST['other']['real_name'];
    ...
    if (!isset($real_name))
    {
         //show_message($_LANG['passport_js']['real_name_invalid']);
    }
    if (!is_email($email))
    {
...
/* 查看订单列表 */
elseif ($action == 'solution_list')
{
    include_once(ROOT_PATH . 'includes/lib_transaction.php');

    $page = isset($_REQUEST['page']) ? intval($_REQUEST['page']) : 1;

    $record_count = $db->getOne("SELECT COUNT(*) FROM " .$ecs->table('solution_order'). " WHERE user_id = '$user_id'");

    $pager  = get_pager('user.php', array('act' => $action), $record_count, $page);

    $orders = get_user_solutions($user_id, $pager['size'], $pager['start']);
...
elseif ($action == 'order_detail')
{
    ...
    $order['pay_status_num'] = $order['pay_status'];
...
elseif ($action == 'act_add_message')
{
    ...
    {
        show_message($_LANG['add_message_success'], $_LANG['message_list_lnk'], 'index.php','info');
    }
    else
    {
...
/* 代为会员注册界面 */
elseif($action == 'send_register')
{

	$smarty->assign('register_title', 'Digidesign 课程注册');
    $smarty->display('user_passport.dwt');
}
/* 注册会员的处理 
elseif ($action == 'act_send_register')
{
    include_once(ROOT_PATH . 'includes/lib_common.php');
	$course 	= isset($_POST['course']) ? trim($_POST['course']) : '';
	$course 	= "所选课程:".$course;
    $pro_username 	= isset($_POST['username']) ? trim($_POST['username']) : '';
	$username	= "<br>姓名:".$pro_username;
    $sex 		= isset($_POST['sex']) ? trim($_POST['sex']) : '';
	$sex		= "<br>性别:".$sex;
    $id_card 	= isset($_POST['id_card']) ? trim($_POST['id_card']) : '';
	$id_card  	= "<br>身份证:".$id_card;
    $mobile_phone = isset($_POST['username']) ? trim($_POST['username']) : '';
	$mobile_phone = "<br>手机:".$mobile_phone;
    $office_phone = isset($_POST['office_phone']) ? trim($_POST['office_phone']) : '';
	$office_phone = "<br>办公电话:".$office_phone;
    $pro_email  	= isset($_POST['email']) ? ''.trim($_POST['email']) : '';
	$email		= "<br>邮箱:".$pro_email;
    $agency_name  = isset($_POST['agency_name']) ? ''.trim($_POST['agency_name']) : '';
	$agency_name  = "<br>机构/公司:".$agency_name;
    $position   = isset($_POST['position']) ? trim($_POST['position']) : '';
	$position	= "<br>职位:".$position;
    $address    = isset($_POST['address']) ? trim($_POST['address']) : '';
	$address	= "<br>地址:".$address;
    
	$content = $course.$username.$sex.$id_card.$mobile_phone.$office_phone.$email.$agency_name.$position.$address;
//    $send_register    = isset($_POST['send_register']) ? trim($_POST['send_register']) : '0';

    // 发送确认重置密码的确认邮件
	$custom_name = "王宁";
	$customer_email = "wangning@sinemedia.com";
	$subject = "Digidesign课程注册-".$username;
	$is_html = 1;
	$mail_from = $pro_email;
    if (send_mail($custom_name, $customer_email, $subject, $content, $is_html,'false',$mail_from))
    {
        show_message("注册信发送成功.您好,".$pro_username . "。我们的工作人员会尽快和您联系，谢谢。", $_LANG['back_page_up'], 'article.php?id=105', 'info', true);
    }
    else
    {
        $err->show("未发送成功", 'user?act=send_register');
    }
}
*/
